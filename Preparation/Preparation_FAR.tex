...
The analyzer working point $\varphi = 45^\circ$ is set via the DC detector signal.
From the modified Malus relation, Eq.~\ref{eq:malus_modified}, one obtains for $\varphi = 45^\circ$:
\begin{equation}
...
Setting $\varphi$ by searching the maximum of the modulated signal $I_{SS}(\varphi)$ is too inaccurate because the modulation curve is very flat near its maximum.
Small changes in $\varphi$ then produce only tiny changes in $I_{SS}$ which are comparable to noise and reading uncertainties, making the peak position poorly defined.
