...
Because the Verdet constant is uniform within the sample and the rotation depends linearly on $B$, this expression can be rewritten using a length-averaged magnetic field,
\begin{equation}
\theta = V(\lambda)\, B_{\mathrm{avg}}\, L,
\end{equation}
with
\begin{equation}
B_{\mathrm{avg}} = \frac{1}{L} \int_0^L B(z)\, \mathrm{d}z.
\end{equation}
...
Using the averaged field therefore allows the standard Faraday relation $\theta = V B L$ to remain valid even for a non-uniform solenoid field.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{../resources/figures/schematic_setup.png}
