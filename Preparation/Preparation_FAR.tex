...
In the experiment, the Faraday rotation angles are very small and cannot be determined accurately by directly measuring changes in the analyzer angle.
Instead, the rotation is converted into a measurable intensity modulation using a polarimetric detection scheme.

The transmitted intensity behind an analyzer is governed by Malus' law,
\begin{equation}
I = I_0 \cos^2 \phi,
\end{equation}
...
To achieve maximum sensitivity to small rotations, the analyzer is set to $\phi = 45^\circ$.
At this working point, the slope of the intensity with respect to $\theta$ is maximal, so even very small changes in polarization lead to a measurable change in intensity.
For small rotation angles $\theta \ll 1$, the cosine term can be expanded to first order, yielding
\begin{equation}
I(\theta) \approx \frac{I_0}{2} - I_0 \theta.
\end{equation}
The transmitted signal therefore consists of a constant part and a contribution that depends linearly on the rotation angle.
...
As a result, the Faraday rotation also oscillates in time,
\begin{equation}
\theta(t) = \theta_B \sin(\Omega t),
\end{equation}
where $\theta_B$ is the rotation amplitude and $\Omega$ the modulation frequency.
Inserting this into the linearized intensity expression gives
\begin{equation}
...
In practice, the DC component $I_{DC}$ is measured with a multimeter, while the oscillating signal is observed on an oscilloscope.
The oscilloscope typically displays the peak-to-peak value of the oscillation,
\begin{equation}
I_{SS} = 2 I_{AC}.
\end{equation}
Using $I_0 = 2 I_{DC}$, the rotation amplitude can therefore be expressed as
\begin{equation}
\theta_B = \frac{I_{SS}}{4 I_{DC}}.
\end{equation}

In real optical systems, finite polarizer extinction ratios and stress-induced birefringence lead to a non-zero transmitted intensity even for crossed polarizers.
To account for this, a modified form of Malus' law is used,
\begin{equation}
I(\phi) = (I_{\parallel} - I_{\perp}) \cos^2 \phi + I_{\perp},
\end{equation}
where $I_{\parallel}$ and $I_{\perp}$ are the transmitted intensities for parallel and crossed polarizers, respectively.
This leads to the definition of a contrast factor
\begin{equation}
K = \frac{I_{\parallel} - I_{\perp}}{I_{\parallel} + I_{\perp}},
\end{equation}
which corrects for imperfect polarization.

Including this correction, the experimentally relevant expression for the rotation amplitude becomes
\begin{equation}
\theta_B = \frac{1}{K} \frac{I_{SS}}{4 I_{DC}}.
\end{equation}

This modulation technique allows the Faraday rotation to be extracted with high sensitivity from the ratio of the oscillating and constant detector signals, while suppressing slow drifts and noise in the optical system.
