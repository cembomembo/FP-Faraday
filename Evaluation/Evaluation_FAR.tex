...
\subsection{Part 3:}
\subsection{Determination of Faraday Rotation Angles}\label{sec:task9_angles}

To calculate the Faraday rotation angle for each measurement, the ratio of the AC modulation signal $U_{SS}$ to the peak-to-peak intensity difference $(I_{||} - I_{\perp})$ was used. The analyzer was set to the working point of $\phi = 45^\circ$ relative to the extinction position. At this angle, the sensitivity $S$ of the setup corresponds to the maximum slope of the transmission curve.

As derived from Malus' Law, including the measured residual intensity $I_{\perp}$ and maximum intensity $I_{||}$, the intensity is $I(\phi) = I_{\perp} + (I_{||} - I_{\perp}) \cos^2(\phi)$. The sensitivity $S$ at the $45^\circ$ working point is:
\begin{equation}
  S = \left| \frac{dI}{d\phi} \right|_{45^\circ} = I_{||} - I_{\perp}
\end{equation}
The magnetic field induces a small rotation $\theta$, resulting in a signal amplitude change. Since $U_{SS}$ represents the \textit{peak-to-peak} voltage of this modulation (which is twice the signal amplitude), the relationship is:
\begin{equation}
  U_{SS} \approx 2 \cdot (S \cdot \theta) = 2 \cdot (I_{||} - I_{\perp}) \cdot \theta
\end{equation}
Rearranging for the rotation angle yields the final calculation formula:
\begin{equation}
  \theta = \frac{U_{SS}}{2 \cdot (I_{||} - I_{\perp})}
  \label{eq:theta_calculation}
\end{equation}
This equation was used to convert the recorded electrical signals into the physical rotation angle $\theta$ (in radians) for all wavelengths and magnetic field strengths. The raw experimental data, including the effective coil currents $I_{\mathrm{eff}}$, the corresponding detector voltages $U_{DC}$ and $U_{SS}$, and the calibration intensities $I_{||}$ and $I_{\perp}$, are recorded in Table~\ref{tab:rotation_angle}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{l c c c c c}
    ...
    & 0.40 & 2.94  & 42.0  & 2.05 & 127.0 \\
    & 0.80 & 2.95  & 82.4  & 2.05 & 248.0 \\
    & 1.20 & 2.96  & 125.0 & 2.05 & 372.0 \\
    & 1.60 & 2.97  & 162.0 & 2.05 & 492.0 \\
    \midrule
    \multirow{4}{*}{\textbf{470 nm}}
    & 0.40 & 2.75  & 31.0  & 2.64 & 104.0 \\
    & 0.80 & 2.76  & 61.0  & 2.64 & 207.0 \\
    & 1.20 & 2.76  & 92.4  & 2.64 & 307.0 \\
    & 1.60 & 2.76  & 121.0 & 2.64 & 410.0 \\
    \midrule
    \multirow{4}{*}{\textbf{508 nm}}
    & 0.40 & 0.797 & 6.64  & 2.54 & 78.0  \\
    & 0.80 & 0.797 & 7.90  & 2.53 & 155.0 \\
    & 1.20 & 0.795 & 9.50  & 2.54 & 232.0 \\
    & 1.60 & 0.798 & 27.0  & 2.53 & 306.0 \\
    \midrule
    \multirow{4}{*}{\textbf{628 nm}}
    & 0.40 & 2.58  & 17.0  & 2.59 & 51.6  \\
    & 0.80 & 2.58  & 29.3  & 2.59 & 101.0 \\
    & 1.20 & 2.58  & 46.8  & 2.59 & 152.0 \\
    & 1.60 & 2.57  & 62.6  & 2.54 & 203.0 \\
    \bottomrule
  \end{tabular}
...
By applying Eq.~\eqref{eq:theta_calculation} to the measured voltages and converting the coil current to the effective magnetic field $\bar{B}_{\mathrm{eff}}$ (using the calibration from Part 2), the physical rotation angles were determined. Table~\ref{tab:max_theta_results} summarizes the maximum rotation angles $\theta_{\mathrm{max}}$ obtained at the highest applied magnetic field for both materials and all four wavelengths.
...
Uncertainties for the magnetic field $B$ and rotation angle $\theta$ were calculated using Gaussian error propagation. For $B = I_{\mathrm{eff}} \cdot C_{\mathrm{coil}}$ (with $C_{\mathrm{coil}} = 7.72 \pm 0.11\,\mathrm{mT/A}$), the uncertainty $\sigma_B$ is:
\begin{equation}
  \sigma_B = B \cdot \sqrt{{\left(\frac{\sigma_{I}}{I_{\mathrm{eff}}}\right)}^2 + {\left(\frac{\sigma_C}{C_{\mathrm{coil}}}\right)}^2}
  \label{eq:error_B}
\end{equation}
where $\sigma_I$ is the coil current reading uncertainty.

Similarly, for the rotation angle $\theta = U_{SS} / (4 \cdot U_{DC})$, the uncertainty $\sigma_\theta$ is derived from the voltage reading uncertainties (the exact factor of $4$ does not contribute):
\begin{equation}
  \sigma_\theta = \theta \cdot \sqrt{\left(\frac{\sigma_{U_{SS}}}{U_{SS}}\right)^2 + \left(\frac{\sigma_{U_{DC}}}{U_{DC}}\right)^2}
