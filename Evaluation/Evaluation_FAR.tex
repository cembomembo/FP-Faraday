...
This quantity is denoted by $I_{SS}$, where ``SS'' refers to the peak-to-peak (Spitze-Spitze) swing of the signal.
...
\begin{equation}
  \bar{B} \approx \frac{1}{L} \sum_{i=1}^{N} B(z_i)\,\Delta z \,=\, \frac{\Delta z}{L} \sum_{i=1}^{N} B(z_i).
...
\begin{equation}
  \bar{B}_{\mathrm{Ampl}} = \sqrt{2}\,\bar{B}.
...
According to Eq.~\ref{eq:sellmeier_linear}, these two quantities follow a linear relation
\begin{equation}
  y = -A\,x + B.
\end{equation}
...
From the measured values $B(z_i)$, the length-averaged magnetic flux density $\bar{B}$ is calculated using Eq.~\ref{eq:Bavg_discrete}.
...
$40\,\mathrm{Hz} \le f \le 90\,\mathrm{Hz}$ ($\Omega = 2 \pi f$).
