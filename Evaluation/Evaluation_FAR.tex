...
\end{equation}
This slope represents the conversion factor between the rotation angle $\theta$ (induced by the Faraday effect) and the measured change in intensity. A extinction angle of $\phi_{0}=90.3\pm0.1^\circ$ was calculated for this specific setup.
...
As shown in Figure~\ref{fig:angle_vs_B}, the measured Faraday rotation angle $\theta$ shows a linear dependence on the magnetic field amplitude $\bar{B}_{\mathrm{Ampl}}$ for all wavelengths. To extract the Verdet constants, a weighted linear regression ($\theta = m \cdot \bar{B}_{\mathrm{Ampl}}$) was applied to the data for each wavelength and material.
...
While most measurements were without major complications and resulted in data with values that are to be expected, the measurement for 508 nm with the N-BK7 glass \ref{fig:angle_vs_B} shows major deviations from the expected linear correlation between the strength of the magnatic field and the angle of rotation. That is because during this measurement the peak-to-peak voltage $U_{SS}$ on the oscilloscope was fluctuating and it was not possible to create a stable wave, resulting in a high signal to noise ratio. This was likely due to a misconfiguration of the oscilloscope, which was then solved for later measurements.\\
