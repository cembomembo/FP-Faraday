...
\begin{equation}
  I(\varphi) = (I_{\parallel} - I_{\perp}) \cos^2 \varphi + I_{\perp},
...
\begin{equation}
  I_{DC}(\varphi = 45^\circ) = \frac{1}{2}\left(I_{\parallel} + I_{\perp}\right).
  \label{eq:IDC_workingpoint}
\end{equation}
This relation provides a precise experimental criterion for setting the analyzer angle.
For each wavelength, $I_{\parallel}$ and $I_{\perp}$ are measured, the corresponding midpoint value of $I_{DC}$ is calculated, and the analyzer is adjusted until the multimeter reading matches this value.

Setting the working point by searching for the maximum of the modulated signal $I_{SS}(\varphi)$ is unsuitable, because the modulation amplitude exhibits a broad and flat maximum around $\varphi = 45^\circ$.
Near this maximum, small changes of the analyzer angle lead to only very small variations of $I_{SS}$, which are comparable to noise and reading uncertainties.
As a result, the position of the maximum cannot be determined with sufficient precision.

In contrast, the DC signal varies monotonically with $\varphi$ in the vicinity of $45^\circ$, and the midpoint condition~\eqref{eq:IDC_workingpoint} therefore allows a much more accurate and reproducible adjustment of the analyzer angle.
...
In practice, the crossed setting is not perfectly dark, i.e. $I_{\perp}\neq 0$.
...
\end{equation}
is introduced.
It describes how strongly the analyzer can modulate the detected intensity between the parallel and crossed configurations.
...
For each wavelength $\lambda_i$, the data are transformed into
\begin{equation}
  x_i = \frac{1}{\lambda_i^2},
  \qquad
  y_i = \frac{1}{n(\lambda_i)^2 - 1}.
\end{equation}

According to Eq.~\ref{eq:sellmeier}, these two quantities follow a linear relation
\begin{equation}
  y = -A\,x + B.
\end{equation}
For each glass type, the values $y_i$ are therefore plotted versus $x_i$, and a linear fit is performed to obtain the slope $(-A)$ and the intercept $B$.

From the fit parameters, the effective resonance wavelength of the one-resonance model is determined as
\begin{equation}
  \lambda_R = \sqrt{\frac{A}{B}} \;=\; \sqrt{\frac{-\text{slope}}{\text{intercept}}}.
\end{equation}

The parameter $A$ (or equivalently the absolute value of the fitted slope) is retained for later evaluation tasks, where it enters the determination of the dispersion-electron density $N$ via Eq.~\ref{eq:N_from_slope}.

\subsection{Part 1: Initial measurements and setup}
...
\subsubsection{Average Magnetic Flux}
...
\subsection{Part 2: Calibration and Malus' Law}
\subsubsection{Malus' Law verification without glass block}
...
For each analyzer angle $\varphi$, the transmitted intensity $I(\varphi)$ is measured and the resulting curve is recorded.
...
\subsubsection{Malus' Law verification with glass block}
...
\subsection{Part 3: Experimental measurements}
\subsubsection{Peak-to-Peak measurement at variable angle}
...
The analyzer working point $\varphi = 45^\circ$ is set via the DC detector signal, as described in~\ref{sec:contrast_correction}.
Experimentally, $I_{\parallel}$ and $I_{\perp}$ are measured with parallel and crossed polarizers, and the analyzer angle is adjusted until the multimeter reads the midpoint value given by Eq.~\ref{eq:IDC_workingpoint}.

For both glass blocks, with the analyzer adjusted to the working point, the analyzer angle $\varphi$ is varied and for each angle the peak-to-peak amplitude $I_{SS}$ is recorded on the oscilloscope.
The values $I_{SS}(\varphi)$ are plotted and the maximum at $\varphi \approx 45^\circ$ is verified.

\subsubsection{Intensity measurements with LEDs setup}
For each of the five LED wavelengths, the Faraday rotation is measured for four different magnetic field strengths.

The analyzer is set to the working point $\varphi = 45^\circ$ using the DC criterion described in Task~8, and the magnetic field strength is varied by adjusting the frequency generator settings.
...
The recorded values from both glass block measurements are used to determine the Faraday rotation amplitude $\theta_B$ for each wavelength and magnetic field strength using the signal-extraction relation from the Physical Principles section, including the contrast correction factor $K$.

\subsection{Part 4: Data analysis}
\subsubsection{Verdet constant and effective mass determination}
...
\subsubsection{Electron density determination}

From the slope $A$ of the linear dispersion plot $1/(n^2-1)$ versus $1/\lambda^2$ obtained in the preparation, and using the effective electron mass $m_e^*$ determined, the density $N$ of dispersion electrons is calculated according to Eq.~\ref{eq:N_from_slope}.
...
\subsection{Determination of the Effective Resonance Wavelength}
As stated in Equation~\ref{eq:sellmeier} and~\ref{eq:linear}, the relationship between the refractive index and the wavelength far from resonance can be simplified to:
\begin{equation}
...
\subsubsection{Results for N-BK7}

The analysis of the N-BK7 dispersion data in the visible range yields a highly linear relationship, as shown in Figure~\ref{fig:nbk7_plots}. The results of the linear fit and the derived physical parameters are summarized in Table~\ref{tab:task1_results}.
