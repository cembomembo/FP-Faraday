...
To calculate the Faraday rotation angle for each measurement, the ratio of the AC modulation signal $U_{SS}$ to the DC background signal $U_{DC}$ was used. The analyzer was set to the working point of $\phi = 45^\circ$ relative to the extinction position. At this angle, the transmitted intensity $I$ is approximately half the maximum intensity ($I \approx I_0/2$). Consequently, the full intensity can be expressed as $I_0 \approx 2 \cdot U_{DC}$.

The sensitivity $S$ of the setup corresponds to the slope of the transmission curve. As derived from Malus' Law, this slope is maximal at the $45^\circ$ working point and equal to the full intensity $I_0$:
\begin{equation}
  S = \left| \frac{dI}{d\phi} \right|_{45^\circ} \approx I_0 \approx 2 \cdot U_{DC}
\end{equation}
...
\begin{equation}
  U_{SS} \approx 2 \cdot (S \cdot \theta) = 2 \cdot (2 U_{DC} \cdot \theta) = 4 \cdot U_{DC} \cdot \theta
\end{equation}
Rearranging for the rotation angle yields the final calculation formula:
\begin{equation}
  \theta = \frac{U_{SS}}{4 \cdot U_{DC}}
  \label{eq:theta_calculation}
\end{equation}
This equation was used to convert the recorded electrical signals into the physical rotation angle $\theta$ (in radians) for all wavelengths and magnetic field strengths. The raw experimental data, including the effective coil currents $I_{\mathrm{eff}}$ and the corresponding detector voltages $U_{DC}$ and $U_{SS}$, are recorded in Table~\ref{tab:rotation_angle}. The coil current $I_{\mathrm{eff}}$ induces a magnetic field, while $U_{\mathrm{DC}}$ and $U_{\mathrm{SS}}$ represent the DC background intensity and the AC modulation signal, respectively.
