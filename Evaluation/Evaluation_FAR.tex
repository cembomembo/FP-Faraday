...
The Faraday effect is a magneto-optical phenomenon in which the plane of polarization of linearly polarized light rotates while passing through a transparent medium in an axial magnetic field, as illustrated schematically in Fig.~\ref{fig:faraday_effect_schematic}.
For a given wavelength and material, the rotation angle $\theta$ is proportional to the magnetic flux density $B$ and the path length $L$ in the medium, \mbox{$\theta = VBL$}, where $V$ is the material- and wavelength-dependent Verdet constant.
...
\includegraphics[width=0.85\linewidth]{../resources/figures/faraday_effect.png}
  ...
  The total rotation angle is proportional to the magnetic field strength and the optical path length.
  Adapted from~\cite{wikimedia_faraday_effect}.}
...
In transparent optical glasses, the dominant electronic resonances relevant for the visible-range dispersion lie in the ultraviolet.
...
\]
where $\Delta\lambda$ denotes the small Zeeman-induced shift of the underlying electronic resonance and is proportional to the applied magnetic field $B$.
It does not represent a change of the wavelength of the propagating light itself, but rather a shift of the resonance that determines the dispersive optical response.
...
In this approach, the Zeeman splitting of circular electron motion results in the so-called Becquerel relation~\cite{FARAnleitung},
\begin{equation}
...
The transmitted intensity behind the analyzer is described by Malus' law~\cite{FARAnleitung},
\begin{equation}
  I = I_0 \cos^2 \varphi,
  \label{eq:malus_law}
\end{equation}
...
For this purpose, the analyzer is set to $\varphi = 45^\circ$, because at this angle the slope of the $\cos^2$ function is maximal and approximately linear in the vicinity of the working point.
...
\begin{figure}[H]
\centering
\includegraphics[width=1.0\linewidth]{../resources/figures/malus_slope.png}
...
This can be caused by imperfect polarizers, slight misalignments, stray light, or partial depolarization of the transmitted beam inside the sample.
To account for this, a modified form of Malus' law is used~\cite{FARAnleitung},
\begin{equation}
...
\end{equation}
is introduced~\cite{FARAnleitung}.
It quantifies the effective contrast of the optical system between the parallel and crossed configurations.
...
For a homogeneous magnetic field, the Faraday rotation is described by
\[
...
As discussed in Sec.~\ref{sec:microscopic_origin}, the Faraday effect is governed by the dispersive optical response of bound electrons and is therefore linked to the wavelength dependence of the refractive index.
To extract microscopic material parameters from the measured Faraday rotation, we now introduce an explicit model for the wavelength dependence of $n(\lambda)$ in transparent optical glasses.
In such materials, light in the visible range does not excite real electronic transitions.
Instead, the optical field drives bound electrons at frequencies far below the relevant electronic resonance frequencies, which lie in the ultraviolet.
...
Restricting the description to the field-free background dispersion then leads to a simple one-resonance model for the refractive index~\cite{FARAnleitung},
\[
...
\]
Taking the reciprocal of both sides gives the so called Sellmeier equation~\cite{FARAnleitung}
\begin{equation}
...
For each wavelength $\lambda$ of the employed LEDs, the corresponding refractive index $n(\lambda)$ is obtained from the RefractiveIndex.INFO database~\cite{refractiveindexinfo}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{../resources/figures/setup_faraday.png}
  ...
  Adapted from~\cite{FARAnleitung}.}
...
\titlespacing*{\paragraph}{0pt}{2.0ex plus .5ex minus .2ex}{0.8ex plus .2ex}

% ------------------
% --- Evaluation ---
% ------------------
\section{Results}
\subsection{Part 1: Preparation and Calibration}
\subsubsection{Determination of the Effective Resonance Wavelength}
\textbf{Note:} The terms $I$ and $U$ are used interchangebly along the document to refer to the same quantity, the intensity measured in Volts, so that $[I]=[\text{Intensity}]=[U]=V$.
...
While the data was initially considered over a broad spectral range (up to 2.5 $\mu$m), the final fit was restricted to the visible and near-visible range ($0.4\,\mu\text{m} \le \lambda \le 0.8\,\mu\text{m}$). This ensures that the parameters $A$ and $\lambda_R$ are optimized for the spectral region where the Faraday rotation is actually measured. Restricting the fit avoids distortions from infrared lattice vibrations, which dominate the dispersive behavior at larger wavelengths and would otherwise skew the single-oscillator model away from the relevant visible-range data.

\paragraph{Results for N-BK7}
To analyze the N-BK7 glass, the refractive index data was extracted from the public database http://refractiveindex.info/. The analysis of the N-BK7 dispersion in the visible range yields a highly linear relationship, as shown in Figure~\ref{fig:nbk7_plots}. The results of the linear fit and the derived physical parameters are summarized in Table~\ref{tab:task1_results}.

\begin{figure}[H]
  \begin{subfigure}{0.9\textwidth}
    \includegraphics[width=\textwidth]{../resources/plots/part1/N-BK7_plot.png}
    \caption{Visible range (only section of the above).}
  \end{subfigure}
  \caption{Dispersion analysis and linearization for N-BK7 glass with data from http://refractiveindex.info/. Only wavelenght wihtin the visible range were selected.}
  \label{fig:nbk7_plots}
\end{figure}

\paragraph{Results for SF10}

The SF10 glass shows a significantly higher refractive index and stronger dispersion compared to N-BK7. The linearization over the visible spectrum (Figure~\ref{fig:sf10_plots}) provides the necessary slope $A:=-m$ for the subsequent determination of the electron density $N$ in Task 11.

\begin{figure}[H]
  \centering
%  \begin{subfigure}{0.9\textwidth}
%    \includegraphics[width=\textwidth]{../resources/plots/part1/N-SF10.png}
%    \caption{Full spectral range.}
%  \end{subfigure}
%  \hfill
  \begin{subfigure}{0.9\textwidth}
    \includegraphics[width=\textwidth]{../resources/plots/part1/N-SF10_plot.png}
    %\caption{Visible range.}
  \end{subfigure}
  \caption{Dispersion analysis and linearization for SF10 glass with data from http://refractiveindex.info/. Only wavelenght wihtin the visible range were selected.}\label{fig:sf10_plots}
\end{figure}
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    Material & Slope $m$ ($\mu\text{m}^2$) & Intercept $c$ & Res. $\lambda_R$ (nm) \\
    \hline
    N-BK7 & $-(7.53\pm0.05)\cdot10^{-3}$ & $0.7909\pm0.0002$ & $97.6\pm0.3$ \\
    SF10  & $-(11.80\pm0.06)\cdot10^{-3}$ & $0.5373\pm0.0002$ & $148.2\pm0.4$ \\
  \end{tabular}
\end{table}

\subsubsection{Magnetic-Field Calibration}

\paragraph{\textbf{Axial field profile and length-averaged field.}}
...
Because the field profile varies along the solenoid axis, this positioning uncertainty affects not only the covered integration interval $L$, but also the assignment of the measured field values to the corresponding positions.
A fully rigorous treatment would therefore require additional information about the local shape of $B(z)$ and is beyond the scope of the present evaluation.
In the following, we therefore use a simpler and conservative estimate in which the positional uncertainty is represented by an effective length uncertainty $\sigma_L$, and the resulting uncertainty of $\bar B_{\mathrm{eff}}$ should be understood as an approximate estimate rather than an exact statistical propagation.
The teslameter display shows the magnetic flux density with a resolution of $0.1\,\mathrm{mT}$ (least significant digit), and we therefore use $\sigma_B \approx \pm0.1\,\mathrm{mT}$ as reading uncertainty.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.95\linewidth]{../resources/figures/B_z.png}
  \caption{Magnetic flux density $B$ measured along the coil symmetry axis at a fixed current $I_{\mathrm{eff}}=1.0\,\mathrm{A}$. The dashed curve is a smooth interpolation added only to visualize the shape of the field profile.}\label{fig:B_z}
\end{figure}

From the measured profile, shown in Fig.~\ref{fig:B_z}, the effective length-averaged magnetic field was calculated using the trapezoidal rule with $\Delta z=0.5\,\mathrm{cm}$ and $L=7.0\,\mathrm{cm}$.
The field profile is approximately symmetric about the solenoid center and exhibits a relatively flat maximum near the middle, while the slope becomes steeper toward the edges.
This also illustrates why a positional uncertainty in $z$ does not translate into the same field uncertainty everywhere along the profile.
For the $N=15$ measurement points, the discrete approximation of the integral is:
\begin{equation}
  \bar B_{\mathrm{eff}} = \frac{\Delta z}{L} \left( \frac{B(z_1)}{2} + \sum_{i=2}^{N-1} B(z_i) + \frac{B(z_N)}{2} \right)
\end{equation}

Assuming independent teslameter readings with identical uncertainty $\sigma_B$, standard Gaussian error propagation for the $N$ terms yields the contribution from the field readings:
\begin{equation}
  \sigma_{\bar B,B} = \frac{\Delta z}{L} \sigma_B \sqrt{ \left(\frac{1}{2}\right)^2 + (N-2) + \left(\frac{1}{2}\right)^2 } = \frac{\Delta z}{L} \sigma_B \sqrt{N - 1.5}
\end{equation}

In addition, the manual positioning of the probe marker introduces an uncertainty in the covered integration length $L$, which we estimate as $\sigma_L \approx 0.1\,\mathrm{cm}$.
...
As discussed above, this treatment of the positional uncertainty is only approximate, since a shift in $z$ also changes the sampled field values through the slope of the profile $B(z)$.

Evaluating this for the data in Table~\ref{tab:Bz_profile_1A} gives:
\begin{equation}
  \boxed{\bar B_{\mathrm{eff}}(I_{\mathrm{eff}}=1.0\,\mathrm{A}) = (7.45 \pm 0.11)\,\mathrm{mT}} \;.
  \label{eq:Bavg_1A_value}
\end{equation}

\paragraph{\textbf{Field-current calibration.}}

To determine the effective length-averaged magnetic field for arbitrary current settings, a calibration curve $\bar B(I_{\mathrm{eff}})$ was required.
Measuring the full axial profile $B(z)$ for every current setting would have been unnecessarily time-consuming.
We therefore fixed the Hall probe at the solenoid center ($z=3.5\,\mathrm{cm}$) and recorded the center field $B_{\mathrm{center}}$ as a function of the effective coil current $I_{\mathrm{eff}}$.

At $I_{\mathrm{eff}}=1.0\,\mathrm{A}$, the measured center field was $B_{\mathrm{center}}=9.3\,\mathrm{mT}$, while the corresponding length-averaged field obtained from the full profile was $\bar B_{\mathrm{eff}}=7.45\,\mathrm{mT}$.
This gives the empirical ratio
\[
  \frac{\bar B_{\mathrm{eff}}}{B_{\mathrm{center}}} = \frac{7.45}{9.3} \approx 0.80.
\]
Assuming that the shape of the axial field profile remains approximately unchanged when the current is varied, we therefore use
\[
  \bar B_{\mathrm{eff}} \approx 0.80\,B_{\mathrm{center}}
\]
to convert the center-field measurements into estimates of the effective length-averaged magnetic field.
In the numerical evaluation, the more precise ratio $7.45/9.3$ was used.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{../resources/figures/Bcenter_calibration_fit.png}
  \caption{Estimated length-averaged effective magnetic field $\bar B_{\mathrm{eff}}$ as a function of effective coil current $I_{\mathrm{eff}}$, obtained from the measured center-field values using $\bar B_{\mathrm{eff}} \approx 0.80\,B_{\mathrm{center}}$, together with a linear regression fit.}
  \label{fig:Bcenter_calibration_fit}
\end{figure}

As shown in Fig.~\ref{fig:Bcenter_calibration_fit}, a linear regression of these estimated length-averaged field values yields the calibration relation
\begin{equation}
  \boxed{\bar B_{\mathrm{eff}}(I_{\mathrm{eff}}) = (7.93 \pm 0.03)\,\frac{\mathrm{mT}}{\mathrm{A}}\,I_{\mathrm{eff}} - (0.49 \pm 0.03)\,\mathrm{mT}} \;,
...
The small negative intercept is consistent with the measured offset at $I_{\mathrm{eff}}=0$ (Figure~\ref{fig:Bcenter_calibration_fit}, Table~\ref{tab:Bcenter_Ieff}).
Since only one zero-current point and only positive currents were recorded, the exact origin of this offset cannot be identified unambiguously.
Possible causes are a residual Hall-probe zero offset, ambient background magnetic fields, or a small residual current in the solenoid circuit.

\subsection{Part 2: Malus' Law and Magnetic field modulation}
\subsubsection{Verification of Malus' Law (Air)}

To verify the polarization properties of the optical setup, the transmitted intensity $I(\phi)$ was measured as a function of the analyzer angle $\phi$ without the glass sample. The data was fitted using the theoretical model for Malus' Law, including a background intensity term:
\begin{equation}
  I(\phi) = I_0 \cdot \cos^2(\phi - \phi_0) + I_{bg}
  \label{eq:malus_model}
\end{equation}
where $I_0$ represents the signal amplitude, $\phi_0$ the phase offset of the analyzer, and $I_{bg}$ the background intensity caused by ambient light or imperfect extinction.

The experimental data and the corresponding fit were recorded in Table~\ref{tab:malus_measurements}, and shown in Figure~\ref{fig:malus_air}. The maximum intensity $I_{\parallel}$ and minimum intensity $I_{\perp}$ were derived from the fit parameters:
\begin{align}
  I_{\parallel} &= I_0 + I_{bg} \\
  I_{\perp} &= I_{bg}
\end{align}
Using these values, the Contrast $K$ and the Degree of Polarization $G$ were calculated:
\begin{equation}
  K = \frac{I_{\parallel} - I_{\perp}}{I_{\parallel} + I_{\perp}}, \quad G = \frac{I_{\parallel}}{I_{\perp}}
\end{equation}
The results for the setup without glass are summarized in Table~\ref{tab:polarization_results}. The errors for $K$ and $G$ are derived from the covariance matrix of the fit using standard error propagation rules.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{../resources/plots/part2/malus_law_without_glass.png}
  \caption{Verification of Malus' Law without the glass sample. The red curve indicates the fit according to Eq.~\ref{eq:malus_model}.}\label{fig:malus_air}
\end{figure}

\subsubsection{Verification of Malus' Law (With Glass)}
The measurement was repeated with the flint glass rod inserted into the coil. The resulting intensity curve is shown in Figure~\ref{fig:malus_glass}. The presence of the glass medium introduces optical interfaces which may lead to scattering, reflections, or stress-induced birefringence.

\begin{table}[htbp]
  \centering
  \begin{tabular}{l c c}
    \toprule
    \textbf{Parameter} & \textbf{Air} & \textbf{Glass Sample} \\
    \midrule
    $I_{\parallel}$ [mV] & 5.145 $\pm$ 0.021 & 5.154 $\pm$ 0.021 \\
    $I_{\perp}$ [mV]     & 0.048 $\pm$ 0.008 & 0.099 $\pm$ 0.008 \\
    Contrast $K$         & 0.982 $\pm$ 0.003 & 0.962 $\pm$ 0.003 \\
    Degree of Pol. $G$   & 107.9 $\pm$ 18.4 & 52.0 $\pm$ 4.3\\
    \bottomrule
  \end{tabular}
  \caption{Comparison of polarization parameters with and without the glass sample. Uncertainties are derived from the covariance of the fit.}\label{tab:polarization_results}
\end{table}

% Sensitivity / Slope Calculation
To determine the optimal working point for the Faraday effect measurements, the sensitivity of the setup was analyzed. The sensitivity is defined as the slope of the transmission curve. Differentiating Eq.~\ref{eq:malus_model} with respect to $\phi$:
\begin{equation}
  S(\phi) = \left| \frac{dI}{d\phi} \right| = \left| I_0 \sin(2(\phi-\phi_0)) \right|
\end{equation}
The maximum sensitivity occurs at a relative angle of $\phi_0=45^\circ$. The uncertainty of the slope $\sigma_S$ is derived using Gaussian error propagation. The error is calculated as:
\begin{equation}
  \sigma_S = \sqrt{ \left( \frac{\partial S}{\partial I_0} \right)^2 \sigma_{I_0}^2 + \left( \frac{\partial S}{\partial \phi_0} \right)^2 \sigma_{\phi_0}^2 }
\end{equation}
Using the fit parameters and their respective uncertainties from the measurement with the glass sample, the slope at the working point is determined to be:
\begin{equation}
  S_{45^\circ} = 0.0882 \pm 0.0002 \, \text{mV}/^\circ
\end{equation}
This slope represents the conversion factor between the rotation angle $\theta$ (induced by the Faraday effect) and the measured change in intensity. A extinction angle of $\phi_{0}=0.3\pm0.1^\circ$ was calculated for this specific setup.

% --- Task 7---
\subsubsection{Verification of Setup Quality}
Task 7 requires an optical adjustment such that $G > 40$. As shown in Table~\ref{tab:polarization_results}, the setup with the glass sample achieved a degree of polarization of $G = 52.0 \pm 4.3$, which confirms that the condition $G > 40$ was satisfied and the optical axis was aligned.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../resources/plots/part2/malus_law_with_glass.png}
  \caption{Verification of Malus' Law with the glass sample. The tangent (orange dashed line) indicates the maximum sensitivity at the working point $\phi = 45^\circ$.}\label{fig:malus_glass}
\end{figure}

\subsubsection{Verification of the Working Point}

To verify the optimal working point for the Faraday effect measurements, the magnetic field was modulated with a frequency of $f = 60$\,Hz. The peak-to-peak amplitude of the resulting modulation signal, $U_{SS}$, was measured as a function of the analyzer angle $\phi$.

The modulation signal is physically proportional to the derivative of Malus' Law. Small rotations $\delta\phi$ induced by the magnetic field lead to intensity changes $\delta I$ according to:
\begin{equation}
  U_{SS}(\phi) \propto \left| \frac{dI}{d\phi} \right| \delta\phi = \left| I_0 \sin(2(\phi - \phi_0)) \right| \delta\phi
\end{equation}

The experimental data, recorded in Table~\ref{tab:modulation_data}, were fitted using a non-linear least squares method with the model function:
\begin{equation}
  U_{model}(\phi) = A \cdot |\cos(2(\phi - \phi_{peak}))| + U_{offset}
\end{equation}
where $A$ represents the maximum modulation amplitude and $\phi_{peak}$ the angle of maximum sensitivity. This fit is shown in Figure~\ref{fig:modulation_signal}.

The uncertainties for these parameters were derived from the variance-covariance matrix $P$ of the fit. The standard error $\sigma_i$ for each parameter $p_i$ (specifically $A$ and $\phi_{peak}$) is calculated as the square root of the corresponding diagonal element:
\begin{equation}
  \sigma_{p_i} = \sqrt{P_{ii}}
\end{equation}
This statistical error accounts for the residual scatter of the measured data points around the theoretical model.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../resources/plots/part2/field_modulation.png}
  \caption{Amplitude of the modulation signal $U_{SS}$ as a function of the analyzer angle $\phi$. The maximum signal indicates the point of highest sensitivity.}\label{fig:modulation_signal}
\end{figure}

The maximum modulation signal was detected with an amplitude of $U_{SS} = 40.06 \pm 0.59$\,mV at an angle of $\phi_{peak} = 46.59^\circ \pm 0.08^\circ$, with an $U_{\text{offset}}\approx0\pm0.6$mV. Since the signal-to-noise ratio was quite high, the background $U_{\text{offset}}$ in the fit resulted in a numerical value of zero.

\subsection{Part 3: Verdet constant and Electron Density}
\subsubsection{Determination of Faraday Rotation Angles}\label{sec:task9_angles}

To calculate the Faraday rotation angle for each measurement, the modulation method was used at the working point of $\phi = 45^\circ$. For each wavelength, the maximum ($U_{||}$) and minimum ($U_{\perp}$) detector voltages were measured to determine the contrast factor $K = \frac{U_{||} - U_{\perp}}{U_{||} + U_{\perp}}$. During the measurements with the modulated magnetic field, the constant DC offset voltage $U_{DC}$ and the peak-to-peak AC modulation voltage $U_{SS}$ were recorded alongside the effective coil current $I_{\mathrm{eff}}$.

Before each measurement series, the detector background voltage $U_{bg}=0.008$V was measured with the beam path blocked. This background signal was subtracted from all static DC voltage measurements to remove the contribution of ambient light.

The physical rotation angle $\theta$ (in radians) is then determined using the final relation:
\begin{equation}
  \theta = \frac{1}{K} \frac{U_{SS}}{4 U_{DC}}
  \label{eq:theta_calculation}
\end{equation}

This equation was applied across all wavelengths and magnetic field strengths. The raw experimental data are recorded in Table~\ref{tab:rotation_angle} and Table~\ref{tab:contrast_data}. By converting the applied coil current to the effective length-averaged magnetic field $\bar{B}_{\mathrm{eff}}$ (using the calibration from Part 2), the rotation angles as a function of the magnetic field were obtained and plotted against the magnetic flux amplitude in Figure~\ref{fig:angle_vs_B}.

\begin{equation}
  \bar{B}_{\mathrm{Ampl}} = \sqrt{2}\bar{B}_{\mathrm{eff}}
\end{equation}

Uncertainties for the magnetic field $B$ and rotation angle $\theta$ were calculated using Gaussian error propagation. Based on the linear calibration curve $\bar B(I_{\mathrm{eff}}) = a \cdot I_{\mathrm{eff}} + b$ (with slope $a = 7.92 \pm 0.03\,\mathrm{mT/A}$ and intercept $b = -0.49 \pm 0.03\,\mathrm{mT}$), the uncertainty $\sigma_B$ is given by:
\begin{equation}
  \sigma_B = \sqrt{a^2 \sigma_I^2 + I_{\mathrm{eff}}^2 \sigma_a^2 + \sigma_b^2}
  \label{eq:error_B}
\end{equation}
where $\sigma_I$ is the reading uncertainty of the effective coil current, and $\sigma_a$ and $\sigma_b$ are the standard errors of the fitted slope and intercept, respectively.

Similarly, for the rotation angle $\theta = \frac{1}{K} \frac{U_{SS}}{4 \cdot U_{DC}}$, the uncertainty $\sigma_\theta$ is derived from the voltage reading uncertainties and the uncertainty of the contrast factor $K$. Since the exact factor of $4$ drops out during the derivation of the relative error, Gaussian error propagation yields:
\begin{equation}
  \sigma_\theta = \theta \cdot \sqrt{{\left(\frac{\sigma_{U_{SS}}}{U_{SS}}\right)}^2 + {\left(\frac{\sigma_{U_{DC}}}{U_{DC}}\right)}^2 + {\left(\frac{\sigma_K}{K}\right)}^2}
  \label{eq:error_theta}
\end{equation}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{../resources/plots/part3/angle_B.png}
  \caption{Faraday rotation angle $\theta$ as a function of the average magnetic field amplitude $\bar B_{\mathrm{Ampl}}$ for N-BK7 and SF10 glass samples. Measurements were taken at four LED wavelengths: 400, 470, 508, and 628 nm. The dashed lines represent weighted linear fits, the slopes of which are used to determine the Verdet constants $V(\lambda)$ based on the sample length L.}\label{fig:angle_vs_B}
\end{figure}

\subsubsection{Verdet Constant and Effective Oscillator Mass}

As shown in Figure~\ref{fig:angle_vs_B}, the measured Faraday rotation angle $\theta$ shows a linear dependence on the magnetic field amplitude $\bar{B}_{\mathrm{Ampl}}$ for all wavelengths, exept for the 508 nm measurement for the N-BK7 glass \ref{fig:angle_vs_B}, which shows major deviations from the expected linear correlation between the strength of the magnatic field and the angle of rotation. That is because during this measurement the peak-to-peak voltage $U_{SS}$ on the oscilloscope was fluctuating and it was not possible to create a stable wave, resulting in a high signal to noise ratio. This was likely due to a misconfiguration of the oscilloscope, which was then solved for later measurements.\\

To extract the Verdet constants, a weighted linear regression ($\theta = m \cdot \bar{B}_{\mathrm{Ampl}}$) was applied to the data for each wavelength and material.

The slopes $m$ of these linear fits correspond to the rotation per unit of magnetic field. The Verdet constant $V(\lambda)$ was then determined by dividing these slopes (converted from the working units of deg/mT to standard SI units of rad/T) by the length of the glass sample, $L = 7.0\,\mathrm{cm}$:
\begin{equation}
  V(\lambda) = \frac{m}{L}
\end{equation}

This Verdet constant is directly related to the dispersion of the refractive index through the Becquerel relation:
\begin{equation}
  V(\lambda) = -\frac{1}{2}\frac{e}{m_{e}^{*}c}\lambda\frac{dn}{d\lambda}
\end{equation}
where $e$ is the elementary charge, $c$ is the speed of light, and $m_{e}^*$ represents the effective mass of the oscillating dispersion electrons. To evaluate how the lattice environment affects the electrons' response, we rearrange this expression to solve for the ratio of the effective mass to the bare electron mass $m_e$:
\begin{equation}
  \frac{m_{e}^*}{m_{e}} = -\frac{e}{2 m_{e} c V(\lambda)} \lambda \frac{dn}{d\lambda}
\end{equation}

The refractive index derivatives $dn/d\lambda$ for both N-BK7 and SF10 were calculated numerically for the experimental wavelengths using the standard Sellmeier equations and corresponding material coefficients. The extracted slopes, calculated Verdet constants, dispersion derivatives, and the resulting mass ratios are all unified in Table~\ref{tab:verdet_and_mass}.

\subsubsection{Density of Dispersion Electrons}

Using the effective oscillator mass $m_e^*$ determined from the Faraday rotation and the slope parameter $A$ from the linear dispersion model, the density of optically active dispersion electrons $N$ can be calculated. As shown in equation~\ref{eq:N_from_slope}, rearranging the Sellmeier slope parameter yields:
\begin{equation}
  N = \frac{\epsilon_0 m_e^* (2\pi c)^2}{e^2 A}
\end{equation}
where $\epsilon_0$ is the vacuum permittivity, $c$ is the speed of light, and $e$ is the elementary charge.

Using the previously calculated adjusted average effective mass $m_e^*$ for N-BK7 and the overall average $m_e^*$ for SF10, the dispersion electron densities were determined. The required slope parameters $A$ were taken from the earlier dispersion analysis as $0.00753\,\mu\mathrm{m}^2$ for N-BK7 and $0.01180\,\mu\mathrm{m}^2$ for SF10. The resulting calculated electron densities are:
\begin{itemize}
  \item \textbf{N-BK7:} $N \approx 1.70 \times 10^{23}\,\mathrm{cm}^{-3}$
  \item \textbf{SF10:} $N \approx 1.18 \times 10^{23}\,\mathrm{cm}^{-3}$
\end{itemize}

To contextualize this result for N-BK7, the calculated density of dispersion electrons was compared to the approximate atomic density of the glass matrix. Assuming a standard borosilicate crown glass composition dominated by $\mathrm{SiO}_2$ (density $\rho \approx 2.51\,\mathrm{g/cm}^3$, molar mass $M \approx 60.08\,\mathrm{g/mol}$, and 3 atoms per molecule), the theoretical atom density is approximately $7.55 \times 10^{22}\,\mathrm{cm}^{-3}$:
\begin{equation}
  N_{\mathrm{atoms,N-BK7}} = \frac{\rho \cdot N_A \cdot \nu}{M} \approx 7.55 \times 10^{22}\,\mathrm{cm}^{-3}
\end{equation}
Comparing the optically active electron density to the atom density yields a ratio of:
\begin{equation}
  \frac{N_{\mathrm{dispersion}}}{N_{\mathrm{atoms,N-BK7}}} \approx 2.25
\end{equation}
This indicates that, on average, approximately 2.25 electrons per atom in the N-BK7 glass matrix actively participate in the optical dispersion process in the visible spectrum.

Similarly, the density of dispersion electrons for SF10 was compared to the atomic density of its glass matrix. SF10 is a dense flint glass typically composed of lead oxide and silica (approximately 72\% PbO and 28\% SiO$_2$). Using the corresponding material properties (density $\rho \approx 4.28\,\mathrm{g/cm}^3$~\cite{SchottSF10}, average molar mass $M \approx 177.5\,\mathrm{g/mol}$, and $\nu\approx2.28$ atoms per molecular unit), the theoretical atom density is calculated as:
\begin{equation}
  N_{\mathrm{atoms,SF10}} = \frac{\rho \cdot N_A \cdot \nu}{M} \approx 1.417 \times 10^{23}\,\mathrm{cm}^{-3}
\end{equation}

Comparing the calculated density of optically active electrons ($N \approx 1.18 \times 10^{23}\,\mathrm{cm}^{-3}$) to this atom density yields the following ratio:
\begin{equation}
  \frac{N_{\mathrm{dispersion}}}{N_{\mathrm{atoms,SF10}}} \approx 0.83
\end{equation}

This result indicates that, on average, approximately 0.83 electrons per atom in the SF10 glass matrix contribute to the optical dispersion in the visible spectrum. Interestingly, this ratio is significantly lower than that of N-BK7 ($\sim 2.25$), despite SF10 exhibiting a much stronger macroscopic dispersion and a higher Verdet constant.
...
It is interesting to see that at B = 0 tesla, the faraday rotation does not seem to be zero, as can be seen in Figure~\ref{fig:angle_vs_B} and even better in Figure~\ref{fig:angle_vs_B_zoomed}. There are two plausible reasons for this behavior, one could be that the zeropoint for the B field was shifted due to a misconfiguration. This is suggested by fig.~\ref{fig:Bcenter_calibration_fit} and equation~\ref{eq:Bcenter_calibration_fit}, where for no current the magnetic field is negative. That is likely due to a miscalibration of the teslameter. If this is taken into consideration, that can account towards a shift in the magnetic field strength seen in Figures~\ref{fig:angle_vs_B} and~\ref{fig:angle_vs_B_zoomed} of $0.49\cdot\sqrt{2} \pm 0.03 \cdot \sqrt{2} \approx 0.69 \pm 0.04 $.

While the measurement for N-BK7 was not very accurate, mainly because it was the first glass we measured and we still had to figure out how to do it better, for the SF10 glass all the fit lines are crossing at a point around $-0.80 \pm 0.03$ mT. This still leaves a shift of $0.11 \pm 0.07$ mT without explanation, assuming the first reason with the miscalibration is correct. This could be because of other external magnetic fields such as earths or from other lab devices. Likely it is a combination of both, as earths magnetic field is with 0.03 to 0.06 mT \cite{nasa_earth_magnetic_field} not quite strong enough to account for the remaining shift in the field strength, but it is also no longer negligible.

%Also we should mention that on the discussed graphs the crossing point of the fit lines is not exactly at $\theta = 0 $, since all lines still cross at the same $theta$ it has to be a constant offset, but I dont know why or if that actually matters, maybe it is okay like that.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{../resources/plots/part3/angle_B_zoomed.png}
  \caption{Zoomed view of fig. \ref{fig:angle_vs_B}}
  \label{fig:angle_vs_B_zoomed}
\end{figure}

Another remark to make is that while the fit to the maximum amplitude of the modulation signal indicates the highest sensitivity to be at an angle of $\varphi \approx 46.6 ^\circ$, the actual measurement of the amplitude at $\varphi = 45 ^\circ$ has the same value, thus confirming the appropriotness of the used working point.

The comparison between the optically active electron ratios of $2.25$ for N-BK7 and $0.83$ for SF10 indicates fewer `effective' electrons per atom participate in SF10, despite its higher Verdet constant and macroscopic dispersion. This contradiction is explained by the heavy lead ions ($Z=82$) dominating the flint glass. Because lead's outer electrons possess a drastically higher oscillator strength due to their polarizability, a smaller physical number of them is required to drive the massive macroscopic polarization observed.
% ------------------
% --- Conclusion ---
% ------------------
\section{Conclusion}

In this experiment, we investigated the Faraday effect—the rotation of the plane of polarization of linearly polarized light in the presence of an axial magnetic field—using two optical glasses, N-BK7 and SF10. Our goal was to measure the wavelength-dependent Verdet constant and relate it to the dispersion properties of the materials.

\textbf{Dispersion Analysis:}
The refractive index dispersion of both glasses was analyzed. For N-BK7, the effective resonance wavelength was determined to be $\lambda_R = 97.6\,\text{nm}$, while for SF10, it was $\lambda_R = 148.2\,\text{nm}$. These values reflect the dominant electronic resonance governing the optical response in the visible range.

\textbf{Magnetic Field Calibration:}
The axial magnetic field profile of the solenoid was measured, and the length-averaged magnetic field was calibrated as a function of coil current. The calibration curve $\bar{B}(I_{\text{eff}}) = (7.92 \pm 0.03)\,\text{mT/A} \cdot I_{\text{eff}} - (0.49 \pm 0.03)\,\text{mT}$ enabled precise control of the magnetic field during the Faraday rotation measurements.

\textbf{Polarization Optimization:}
Malus' law was verified for both air and the glass samples, confirming the quality of the optical setup. The degree of polarization G exceeded 40 for most measurements, ensuring reliable detection of small Faraday rotation angles. The working point of the analyzer was set to $\varphi = 45^\circ$ to maximize sensitivity, as validated by the modulation signal analysis.

\textbf{Faraday Rotation Measurements:}
The Faraday rotation angle $\theta$ was measured for four wavelengths (400 nm, 470 nm, 508 nm, and 628 nm) and at multiple magnetic field strengths. The results showed a linear dependence of $\theta$ on the magnetic field amplitude, as expected from theory. The Verdet constants $V(\lambda)$ were extracted from the slopes of these linear relationships, revealing a strong wavelength dependence and significantly higher values for SF10 compared to N-BK7.

\textbf{Verdet Constant:}
The results in Table~\ref{tab:verdet_and_mass} show a clear trend: the rotation angle increases significantly as the wavelength decreases (normal dispersion). Furthermore, the rotation in the lead-containing glass (SF10) is consistently larger than in the crown glass (N-BK7) by a factor of approximately 3.3, indicating a much higher Verdet constant for SF10.

\textbf{Density of dispersion electrons:}
The high Verdet constant of SF10 is not due to more participating electrons; its electron-to-atom ratio is actually lower than N-BK7's. Instead, the strong magneto-optical response stems from the highly polarizable valence electrons in lead ions.

Overall, both glasses exhibit the expected macroscopic behavior of normal dispersion: the Verdet constant drops rapidly as the incident wavelength moves further from the materials' respective ultraviolet resonance frequencies. While this inverse wavelength dependence is present in both samples, the absolute magnitude of the magneto-optical response in SF10 remains consistently dominant across the entire spectrum. As shown by the electron density analysis, this superior response is not a result of bulk electron participation, but rather a direct consequence of the extreme efficiency and polarizability of the heavy lead ions acting as classical oscillators.
...
\begin{table}[H]
  ...
  \begin{tabular}{|c|c|}
    ...
    \bottomrule
  \end{tabular}
  \caption{Measured center-field values $B_{\mathrm{center}}$ at $z=3.5\,\mathrm{cm}$ as a function of $I_{\mathrm{eff}}$.}
...
\begin{table}[H]
  ...
  \begin{tabular}{c c c}
    \toprule
    {$\phi$ [$^\circ$]} & {$I_{\text{Glass}}$ [mV]} & {$I_{\text{Air}}$ [mV]} \\
    $\sigma_{\phi} = \pm 0.5$ & $\sigma_{I} = \pm 0.02$ & $\sigma_{I} = \pm 0.02$ \\
    \midrule
    90  & 5.16  & 5.16 \\
    80  & 5.00  & 5.01 \\
    70  & 4.55  & 4.57 \\
    ...
    \bottomrule
  \end{tabular}
  \caption{Measured intensities for the verification of Malus' Law, with and without the glass sample.}\label{tab:malus_measurements}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{c c}
    \toprule
    {$\phi$ [$^\circ$]} & {$U_{SS}$ [mV]} \\
    $\sigma_{\phi} = \pm 0.5$ & $\sigma_{U_{SS}} = \pm 0.1$ \\
    \midrule
    30 & 33.0 \\
    35 & 36.0 \\
    38 & 38.4 \\
    ...
    \bottomrule
  \end{tabular}
  \caption{Measurement of the modulation signal amplitudes $U_{SS}$ at analyzer angles $\phi$.}\label{tab:modulation_data}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{l c c c c c}
    \toprule
    \multirow{2}{*}{\textbf{Wavelength}} & \textbf{$I_{\mathrm{eff}}$} & \multicolumn{2}{c}{\textbf{N-BK7}} & \multicolumn{2}{c}{\textbf{SF10}} \\
    \cmidrule(lr){3-4} \cmidrule(lr){5-6}
    & [A] & $U_{\mathrm{DC}}$ [V] & $U_{\mathrm{SS}}$ [mV] & $U_{\mathrm{DC}}$ [V] & $U_{\mathrm{SS}}$ [mV] \\
    & $\sigma_{I_{\mathrm{eff}}}=\pm$ 0.01 & $\sigma_{U_{\mathrm{DC}}}=\pm$ 0.01 & $\sigma_{U_{\mathrm{SS}}}=\pm$ 0.1 & $\sigma_{U_{\mathrm{DC}}}=\pm$ 0.01 & $\sigma_{U_{\mathrm{SS}}}=\pm$ 0.1\\
    \midrule
    \multirow{4}{*}{\textbf{400 nm}}
    & 0.40 & 2.93  & 42.0  & 2.04 & 127.0 \\
    & 0.80 & 2.94  & 82.4  & 2.04 & 248.0 \\
    & 1.20 & 2.95  & 125.0 & 2.04 & 372.0 \\
    & 1.60 & 2.96  & 162.0 & 2.04 & 492.0 \\
    \midrule
    \multirow{4}{*}{\textbf{470 nm}}
    & 0.40 & 2.74  & 31.0  & 2.63 & 104.0 \\
    & 0.80 & 2.75  & 61.0  & 2.63 & 207.0 \\
    & 1.20 & 2.75  & 92.4  & 2.63 & 307.0 \\
    & 1.60 & 2.75  & 121.0 & 2.63 & 410.0 \\
    \midrule
    \multirow{4}{*}{\textbf{508 nm}}
    & 0.40 & 0.789 & 6.64  & 2.53 & 78.0  \\
    & 0.80 & 0.789 & 7.90  & 2.52 & 155.0 \\
    & 1.20 & 0.787 & 9.50  & 2.53 & 232.0 \\
    & 1.60 & 0.790 & 27.0  & 2.52 & 306.0 \\
    \midrule
    \multirow{4}{*}{\textbf{628 nm}}
    & 0.40 & 2.57  & 17.0  & 2.58 & 51.6  \\
    & 0.80 & 2.57  & 29.3  & 2.58 & 101.0 \\
    & 1.20 & 2.57  & 46.8  & 2.58 & 152.0 \\
    & 1.60 & 2.56  & 62.6  & 2.53 & 203.0 \\
    \bottomrule
  \end{tabular}
  \caption{Experimental measurement data for the Faraday Effect using different mediums and light sources. Exceptions to the standard errors are the maximum deviation for $I_{\mathrm{eff}}$ at 1.60\,A (628\,nm), which is $\pm 0.02$\,A, and the reading uncertainty for $U_{\mathrm{DC}}$ in the 508\,nm N-BK7 dataset, which is $\pm 0.001$\,V.}\label{tab:rotation_angle}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{l c c c c c c}
    \toprule
    \multirow{2}{*}{\textbf{Wavelength}} & \multicolumn{3}{c}{\textbf{N-BK7}} & \multicolumn{3}{c}{\textbf{SF10}} \\
    \cmidrule(lr){2-4} \cmidrule(lr){5-7}
    & $U_{||}$ [V] & $U_{\perp}$ [V] & $K$ & $U_{||}$ [V] & $U_{\perp}$ [V] & $K$ \\
    & $\sigma_{U_{||}}=\pm$ 0.01 & $\sigma_{U_{\perp}}=\pm$ 0.01 & & $\sigma_{U_{||}}=\pm$ 0.01 & $\sigma_{U_{\perp}}=\pm$ 0.01 & \\
    \midrule
    \textbf{400 nm} & 5.102 & 0.787 & 0.733 & 4.012 & 0.060 & 0.971 \\
    \textbf{470 nm} & 5.082 & 0.355 & 0.869 & 5.252 & 0.222 & 0.919 \\
    \textbf{508 nm} & 4.514 & 0.062 & 0.973 & 4.062 & 0.005 & 0.998 \\
    \textbf{628 nm} & 5.011 & 0.148 & 0.943 & 5.162 & 0.006 & 0.998 \\
    \bottomrule
  \end{tabular}
  \caption{Maximum ($U_{||}$) and minimum ($U_{\perp}$) detector voltages and the resulting calculated contrast factors $K$ for each wavelength and glass sample. Background voltages of $U_{\mathrm{bg}} \approx 0.008$\,V (and $0.009$\,V for N-BK7 at 628\,nm) were subtracted from the raw measurements prior to determining $K$.}\label{tab:contrast_data}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{l c c c c c}
    \toprule
    \textbf{Material} & \textbf{Wavelength [nm]} & \textbf{Slope $m$ [rad/T]} & \textbf{$V(\lambda)$ [rad/(T$\cdot$m)]} & \textbf{$dn/d\lambda$ [m$^{-1}$]} & \textbf{$m_{e}^*/m_{e}$} \\
    \midrule
    \multirow{4}{*}{\textbf{N-BK7}}
    & 400 & $1.044 \pm 0.006$ & $14.91 \pm 0.09$ & $-1.33 \times 10^5$ & $1.048$ \\
    & 470 & $0.708 \pm 0.004$ & $10.11 \pm 0.06$ & $-8.02 \times 10^4$ & $1.093$ \\
    & 508 & $0.361 \pm 0.004$ & $5.16 \pm 0.06$ & $-6.34 \times 10^4$ & $1.832$ \\
    & 628 & $0.347 \pm 0.002$ & $4.96 \pm 0.04$ & $-3.49 \times 10^4$ & $1.298$ \\
    \midrule
    \multirow{4}{*}{\textbf{SF10}}
    & 400 & $3.448 \pm 0.025$ & $49.25 \pm 0.36$ & $-5.47 \times 10^5$ & $1.304$ \\
    & 470 & $2.347 \pm 0.013$ & $33.53 \pm 0.19$ & $-2.83 \times 10^5$ & $1.162$ \\
    & 508 & $1.694 \pm 0.011$ & $24.20 \pm 0.16$ & $-2.12 \times 10^5$ & $1.305$ \\
    & 628 & $1.101 \pm 0.006$ & $15.72 \pm 0.09$ & $-1.03 \times 10^5$ & $1.209$ \\
    \bottomrule
  \end{tabular}
  \caption{Extracted slopes from the linear fits of the Faraday rotation angles, the calculated Verdet constants $V(\lambda)$, refractive index derivatives, and the resulting effective oscillator mass ratios for N-BK7 and SF10 glasses.}\label{tab:verdet_and_mass}
\end{table}
