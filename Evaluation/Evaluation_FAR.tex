...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.9\textwidth}
    \includegraphics[width=\textwidth]{../resources/plots/part1/N-BK7.png}
  ...
  \end{subfigure}
  \caption{Dispersion analysis and linearization for N-BK7 glass with data from http://refractiveindex.info/}
...
%  \hfill
  ...
  \end{subfigure}
  \caption{Dispersion analysis and linearization for SF10 glass with data from http://refractiveindex.info/ for visible range.}\label{fig:sf10_plots}
\end{figure}
...
The small negative intercept is consistent with the measured offset at $I_{\mathrm{eff}}=0$ (Table~\ref{tab:Bcenter_Ieff}).
...
\end{equation}
The maximum sensitivity occurs at a relative angle of $45^\circ$. The uncertainty of the slope $\sigma_S$ is derived using Gaussian error propagation. The error is calculated as:
\begin{equation}
...
Task 7 requires an optical adjustment such that $G > 40$. As shown in Table~\ref{tab:polarization_results}, the setup with the glass sample achieved a degree of polarization of $G = 52.0 \pm 4.3$, which confirms that the condition $G > 40$ was satisfied and the optical axis was correctly aligned.
...
\begin{equation}
  U_{SS}(\phi) \propto \left| \frac{dI}{d\phi} \right| = \left| I_0 \sin(2(\phi - \phi_0)) \right|
\end{equation}

The experimental data were fitted using a non-linear least squares method with the model function:
\begin{equation}
  U_{model}(\phi) = A \cdot |\cos(2(\phi - \phi_{peak}))| + U_{offset}
\end{equation}
where $A$ represents the maximum modulation amplitude and $\phi_{peak}$ the angle of maximum sensitivity.
...
The experimental data are recorded in Table~\ref{tab:modulation_data}, and the fit is shown in Figure~\ref{fig:modulation_signal}.
...
The maximum modulation signal was detected with an amplitude of $U_{SS} = 40.06 \pm 0.59$\,mV at an angle of $\phi_{peak} = 46.59^\circ \pm 0.08^\circ$, with an $U_{\text{offset}}\approx0\pm0.6$mV. Since the SNR was quite high, the background $U_{\text{offset}}$ in the fit resulted in a numerical value of zero.
...
Using the effective oscillator mass $m_e^*$ determined from the Faraday rotation and the slope parameter $A$ from the linear dispersion model, the density of optically active dispersion electrons $N$ can be calculated. Rearranging the Sellmeier slope parameter yields:
\begin{equation}
