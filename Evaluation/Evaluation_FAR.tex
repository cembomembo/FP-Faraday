...
The light source is a set of light-emitting diodes (LEDs) with different central wavelengths in the visible range.
...
After collimation, the light passes through the first polarizer, which defines a well-controlled linear polarization state.
...
As discussed in the Physical Principles section, this magnetic field induces a Faraday rotation of the polarization plane inside the sample.
...
Its transmission axis is set to an angle of $\varphi = 45^\circ$ with respect to the initial polarization direction.
In this configuration, small Faraday-induced polarization rotations are converted into proportional changes in transmitted intensity.

Finally, a second lens focuses the transmitted light onto a photodiode detector.
The photodiode converts the optical intensity into an electrical voltage signal, which contains both a constant component and a small oscillating component at the modulation frequency of the magnetic field.
These signals are recorded using a multimeter and an oscilloscope and form the basis for the determination of the Faraday rotation angle and the Verdet constant.
...
The analyzer working point $\varphi = 45^\circ$ is set via the DC detector signal, as described in Sec.~\ref{sec:contrast_correction}.
